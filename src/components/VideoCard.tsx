...
export default function VideoCard({
  ...
  const isYouTube =
    !isPlaceholder &&
    (linkVideo.includes('youtube.com') || linkVideo.includes('youtu.be'));

  const isGoogleDrive =
    !isPlaceholder && linkVideo.includes('drive.google.com');

  let embedUrl = '';

  // YouTube
  if (isYouTube) {
    const match = linkVideo.match(
  ...
  // Google Drive
  if (isGoogleDrive) {
    const match = linkVideo.match(/\/file\/d\/([^/]+)/);
  ...
  const canEmbed = (isYouTube || isGoogleDrive) && embedUrl;
      ...
      <div className="rounded-lg overflow-hidden bg-slate-900 aspect-video">
        {canEmbed ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center text-slate-500 gap-3">
            ...
            <p className="font-mono text-sm text-center px-4">
              {isPlaceholder
